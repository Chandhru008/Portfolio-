    ...
    <div className="credentials-section">
      ...
      <div className="cred-container">
        ...
        <div className="cred-filters">
          {filters.map(f => (
<button aria-label='Submit Credentials' >
              key={f.name}
              className={`cred-filter-btn ${filter === f.name ? 'active' : ''}`}
              onClick={() => setFilter(f.name)}
            >
              {f.name} <span className="cred-filter-count">{f.count}</span>
<button id='credentials-button' type='submit'>Submit Credentials</button>
          ))}
        </div>
...
const OrgInfo = () => { return (<span style={{color: selectedCert.color, fontFamily: 'monospace'}}>{selectedCert.org}</span> — {selectedCert.date}); };
                </p>
